 ...
 * 매개변수는 메소드를 나오면 소멸된다. 
 ...
 - TreeSet<E> 클래스 
    ...
    - tree에 넣어줄 인스턴스의 정렬기준은 대부분의 상황에서 프로그래머가 직접 정의해야 한다. 
        - Comparable<T> 인터페이스를 구현함으로써 (Wrapper 클래스를 정의해야함)
        interface Comparable<T>
        { int compareTo(T obj) }
        - compareTo메소드는 다음 내용을 근거로 정의되어야 한다. 
            1. 인자로 전달된 obj가 작다면 양의 정수를 반환해라
            2. 인자로 전달된 obj가 크다면 음의 정수를 반환해라
            3. 인자로 전달된 obj와 같다면 0을 반환해라 
        - Comparator<T>의 compare()메소드 사용 
        
 ### Map<K,V> 인터페이스를 구현하는 컬렉션 클래스들 
 - key : 데이터를 찾는 열쇠를 의미
 - value : 찾고자 하는 실질적인 데이터를 의미
 - Map<K,V>를 구현하는 컬렉션 클래스들은 value를 저장할때, 이를 찾는데 사용되는 key도 함께 저장한다. 
 
 1. HashMap<K,V>
 - value에 관계없이 중복된 key의 저장은 불가능
 - value는 같더라도 key가 다르면 둘 이상의 데이터 저장도 가능하다. 
 - 매우 빠른 검색 속도
 
 2. TreeMap<K,V> 클래스
 - 트리 자료구조를 기반으로 구현되어 있음. --> 데이터는 정렬된 상태로 저장된다. 
 - 여기에 저장된 전체 데이터의 참조는 navigableKeySet 메소드를 사용한다. 
 
 # 자바 기본 공부 24
 ## 쓰레드와 동기화
 
 ### 쓰레드의 이해와 생성
 - 프로세스 : 할당된 메모리 공간을 기반으로 실행 중에 있는 프로그램
 - 하나의 프로세스 내에서 둘 이상의 프로그램 흐름을 형성할 수 있음 
 - 쓰레드 : 프로세스 내에서 프로그램의 흐름을 형성하는 주체 / 자신만의 메모리 공간을 할당받아 별도의 실행흐름을 형성한다.
        : Thread 를 상속하는 클래스의 인스턴스 (Runnable 인터페이스의 구현을 통해서도 쓰레드 생성할 수 있음)
        : 별도의 실행흐름을 형성하기 윟서 자바 가상머신에 의해 만들어지는 모든 리소스와 각종정보를 통칭해 쓰레드라고 한다.
 
 - Thread 의 main 메소드 : run()
 - Thread의 static 메소드 : sleep() - 해당 부분에서 1/1000 * 100 초간 흐름을 멈춤
 
 - start()를 통해 run()을 실행하는 이유 : 메모리 공간 할당 등 쓰레드의 실행을 위한 기반을 마련한 다음에 run()메소드를 호출해야하기 때문에.
 - 다름 쓰레드의 실행완료를 기다리는 목적으로 호출되는 join 메소드
 
 ### 쓰레드의 특성
 - 쓰레드의 스케줄링과 쓰레드의 우선순위 컨트롤
    - 스케줄링에 사용되는 기본 원칙
        1. 우선수위가 높은 쓰레드의 실행을 우선한다.
        2. 동일한 우선순위의 쓰레드가 둘 이상 존재할 때는 CPU의 할당시간을 분배해서 실행한다. 
    - 우선순위는 낮은 1부터 높은10 까지 10단계 존재. (프로그램 상에서 변경 및 확인 가능)
    - 우선순위가 높은 쓰레드에게만 실행의 기회를 부여 --> 우선순위가 같은 쓰레드는 거의 실행 X
        but, CPU의 할당이 필요치 않은 입출력을 처리하는 상황에 놓였을때, 자신에게 할당된 CPU를 다른 쓰레드에게 넘긴다. 그래서 우선순위가 낮은 쓰레드역시 실행의 기회를 얻을 수 있는것
 
 - 쓰레드의 라이프 사이클
 ![쓰레드의 라이프 사이클](https://user-images.githubusercontent.com/28684368/43121718-ac333586-8f59-11e8-9866-f28018ed3f20.JPG)
 
    1. new 상태 : 쓰레드의 클래스가 키워드 new를 통해서 인스턴스화된 상태를 말한다.
                : 자바 가상머신에 의해 관리되는 쓰레드상태는 아님
    
    2. Runnable 상태 : 쓰레드 인스턴스를 대상으로 start메소드가 호출되었을때의 상태
                     : 모든 실행의 준비를 마치고, 스케줄러에 의해 선택되어 실행될 수 있기만을 기다리는 상태 
                     : start()를 호출한다고 해서 바로 run()이 실행되는 것은 아님 (선택도야 run() 호출)
    
    3. no Runnable 상태 : 실행중인 쓰레드가 sleep, join 메소드를 호출, 입출력 연산을 하게되면 다른 쓰레드에게 CPU양보 후 이 상태가 된다.
                        : 이 상태에서는 스케줄러의 선택을 받을 수 없음
                       
    4. dead 상태 : run() 의 실행이 완료되어 run() 메소드를 빠져 나오게 되었을 때
                 : 각종 쓰레드 관련 정보가 완전히 사라지는 상태 (다시 Runnable로 돌아갈 수 없음) 
                 
 - 쓰레드의 메모리 구성 
    - main 메소드와는 전혀 다른 실행 흐름을 형성하기 위해서는 별도의 스택이 쓰레드에게 할당되어야 한다. 
    - 메소드 영역 / 스택영역 / 힙 영역 --> 스택영역은 쓰레드마다 가지고 있고 나머지 두개는 공유하는 형태
    - 힙 영역을 공유한다는 것은 
            A쓰레드가 만든 인스턴스의 참조 값(사실상 주소값)만 알면 B 쓰레드도 A쓰레드가 만든 인스턴스에 접근가능하다. 
 ### 동기화
